...
namespace Talent.Services.Profile.Controllers
{
    ...
    public class ProfileController : Controller
    {
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "talent")]
        public async Task<ActionResult> UpdateProfilePhoto([FromForm(Name = "myImage")]IFormFile photo)
        {
            //Your code here;
        ...
        public IActionResult GetTalentSnapshots( int activeFeed, int limit = 10, String id = "")
        {
            try
            {
                
                string userId = String.IsNullOrWhiteSpace(id) ? _userAppContext.CurrentUserId : id;
                var profile =  _employerRepository.GetByIdAsync(userId);
                var result = profile.Result.CompanyContact;
                    var user = _userRepository.GetQueryable()
                 .Skip((activeFeed - 1) * limit)
                 .Take(limit)
                 .Select(u => new User
                 {
                ...
                var  userList = new List<TalentSnapshotViewModel>();
                foreach(var item in user)
                {
                    ...
                    userList.Add(new TalentSnapshotViewModel
                    {
                        Id = item.Id,
                        Name = item.FirstName,
                ...
                return Json(new { Success = true, data = user, result });
            }
            catch (Exception e)
            {
